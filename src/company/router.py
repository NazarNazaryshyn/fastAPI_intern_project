...
async def get_all_companies(current_user = Depends(get_current_user)) -> List[CompanyScheme]:
    async with async_session() as session:
        async with session.begin():
            company_crud_method = CompanyCrudMethod(db_session=session)
...
async def change_visibility(company_id: int, is_visible: bool, current_user = Depends(get_current_user)) -> set:
    async with async_session() as session:
        async with session.begin():
            company_crud_method = CompanyCrudMethod(db_session=session)
...
async def update_company(company_id: int, title: str, description: str, current_user = Depends(get_current_user)) -> CompanyScheme:
    async with async_session() as session:
        async with session.begin():
            company_crud_method = CompanyCrudMethod(db_session=session)
